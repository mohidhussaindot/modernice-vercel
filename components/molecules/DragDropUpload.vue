<template>
  ...
  >
    <input
      type="file"
      :accept="accept"
      class="absolute opacity-0 top-0 left-0 w-full h-full cursor-pointer z-10"
      :id="inputIdValue"
    ...
    <label
      :for="inputIdValue"
      :class="[
        'block cursor-pointer w-full h-full',
        dropzoneItselfClass,
        isDraggingOver ? dragOverClass : ''
      ]"
    >
      <div
        v-if="!objectUrl"
      ...
      <div v-else class="relative w-full h-full flex items-center justify-center">
        ...
        <BaseButton
          ...
          :class="[
            'absolute top-2 right-2 bg-red-500 rounded-full p-1 hover:bg-red-600 transition-colors z-20',
...
<script setup>
  ...
  const isImageFile = computed(() => {
    ...
    if (typeof objectUrl.value === 'string') {
      return (
        objectUrl.value.startsWith('data:image') ||
  ...
  const setCurrentFile = file => {
    ...
    if (file) {
      if (typeof File !== 'undefined' && file instanceof File && isImageFile.value) {
        ...
        typeof file === 'string' &&
        (file.startsWith('data:image') || file.startsWith('http') || file.startsWith('/'))
      ) {
  ...
  onBeforeUnmount(() => {
    if (
      objectUrl.value &&
      typeof objectUrl.value === 'string' &&
      objectUrl.value.startsWith('blob:')
    ) {
      URL.revokeObjectURL(objectUrl.value)
    }
  })
